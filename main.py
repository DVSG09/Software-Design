...
class Player:
    ...
    def __new__(cls):
        if cls.__instance is None:
            cls.__instance=super(Player,cls).__new__(cls)
...
def get_enemies_info(enemy_level=0.0):
    ...
    for each in range(1, num_enemies + 1):
        enemies_info[f'enemy_{each}'] = IconAdapter(Enemy(l[each - 1], enemy_level=0.3 * enemy_level), animate='move')
    return enemies_info
...
def get_candies_info():
    ...
    for each in range(1, num_candies + 1):
        candies_info[f'candy_{each}'] = IconAdapter(Candy(l[each - 1]), animate='show')
    return candies_info
...
def playgame(playerobj, score, candy, hitpoints, level):
    ...
    powerobj = IconAdapter(Power(), animate='fire')
    while running:
        if GAMEOVER:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    if powerobj.powerStatus == 'Ready':
                        ...
                        powerobj.animate(powerobj.powerX, powerobj.powerY)
